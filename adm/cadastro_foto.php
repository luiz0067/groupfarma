...
if ($_POST) {
	...
	if ($_POST ['acao']=='alterar'){
		...
		$sql = "delete FROM fotos  where (fotos.empresa=0".$_SESSION["empresa"].") and (codigo=0".$codigo.")";
		//echo $sql;
		mysql_query($sql, $link);
	}
...
?>
	...
	<form action="?projetos=<?php echo $projetos?>" method="post" enctype="multipart/form-data" name="form1" id="form1">
		<table >
			<tr>
				<td>codigo:</td>
				<td><input name="codigo" type="text" value="<?php echo $codigo?>"></td>
			</tr>
			<tr>
				<td>imagem:</td>
				<td><input name="imagem" type="file" ></td>
			</tr>
			<input name="projetos" type="hidden" value="<?php echo $projetos;?>">
			<tr>
				<td>nome:</td>
...
<?php
			...
			$sql    = "select f.codigo,f.projetos,f.nome,f.imagem,f.link,f.empresa,p.codigo as projetos_codigo,p.descricao as projetos_descricao,p.nome as projetos_nome,p.descricao as projetos_descricacao,p.categoria as projetos_categoria, c.nome as categoria_nome from fotos f left join projetos p on (p.codigo=f.projetos) left join categoria c on (c.codigo=p.categoria) where (f.empresa=".$_SESSION["empresa"].")and(f.projetos=0".$projetos.")";
